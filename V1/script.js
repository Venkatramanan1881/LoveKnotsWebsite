...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (clientsTrack && typeof PORTFOLIO_CLIENTS !== 'undefined') {
    ...
    const scrollContainer = document.querySelector('.clients-scroll-container');
    if (scrollContainer) {
      scrollContainer.addEventListener('wheel', (evt) => {
        // Only override if they are scrolling vertically (deltaY) and not holding shift
        if (evt.deltaY !== 0 && !evt.shiftKey) {
          ...
          if ((evt.deltaY < 0 && canScrollLeft) || (evt.deltaY > 0 && canScrollRight)) {
            evt.preventDefault();
